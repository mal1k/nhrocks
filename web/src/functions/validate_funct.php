	...
	*/
	...
	function validate_form($form, $array, &$error, $bannerErrorSize = 0) {
            ...
            if (strlen($array_options_number[0])) {
                ...
                for ($i = 0; $i < count($array_options_number); $i++) {
                    ...
                    if (($array["dropdown_link_to_".$array_options_number[$i]] != "custom") && in_array($array["dropdown_link_to_".$array_options_number[$i]], $arrayLinksTo)) {
                        /* ModStores Hooks */
                    	if (!HookFire("validate_funct", [
                            "errorArray"           => &$errorArray,
                            "array_options_number" => &$array_options_number,
                            "i"                    => &$i,
                            "array"                => &$array
                        ])) {
		...
		elseif ($form == "listing") {
			...
			$elems[] = array('name'=>'label_additional_phone', 'label'=>system_showText(LANG_LABEL_LABEL_ADDITIONAL_PHONE), 'type'=>'text', 'required'=>false, 'len_max'=>'255');
			$elems[] = array('name'=>'additional_phone', 'label'=>system_showText(LANG_LABEL_ADDITIONAL_PHONE), 'type'=>'text', 'required'=>false, 'len_max'=>'255');
	...
	function validate_listingtemplate($array, &$error, $validateAll = true) {
        ...
        while ($rowCustom = mysqli_fetch_assoc($resultCustom)) {
