<?php
	...
	function template_CreateDynamicField($fieldvalues) {
        ...
        if (string_strpos($fieldvalues["label"], "LANG_LABEL") !== false){
            ...
            switch ($fieldType) {
                ...
                case "custom_short_desc":
                    ...
                    <div class="form-group">
                        <label for="<?=$fieldvalues["field"]?>"><?=$fieldvalues["label"]?> <?=($fieldvalues["required"] == "y") ? "" : "<small class=\"small text-muted\">(optional)</small>"?></label>
                        <textarea id="<?=$fieldvalues["field"]?>" name="<?=$fieldvalues["field"]?>" class="form-control textarea-counter" rows="5" cols="1" <?=($fieldvalues["instructions"]) ? "placeholder=\"".$fieldvalues["instructions"]."\"" : ""?> data-chars="250" data-msg="<?=system_showText(LANG_MSG_CHARS_LEFT)?>"><?=$fieldvalues["form_value"]?></textarea>
                    </div>
                    <?
                break;
                ...
                case "custom_dropdown": ?>
                    <div class="form-group selectize">
                        <label><?=$fieldvalues["label"]?> <?=($fieldvalues["required"] == "y") ? "" : "<small class=\"small text-muted\">(optional)</small>"?></label>
                        ...
                        <? if ($fieldvalues["instructions"]) { ?>
                        <p class="help-block small"><?=$fieldvalues["instructions"]?></p>
                        <? } ?>
                    </div>
                    <?
                break;
