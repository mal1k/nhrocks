...
function system_gamefyItems($item_type, $itemObj)
{
    ...
    if (is_array($array_fields)) {
        ...
        if (in_array('additional_phone', $array_fields)) {
            $arrayAdditional[] = 'additional_phone';
        }
...
function system_getFontAwesomeIcons($return = array())
{
    $icons = new Awps\FontAwesome();
    $icons = $icons->getAllData();
    foreach ($icons as $key => $icon) {
        $return[$key] = '&#x' . str_replace('\\', '', $icon['unicode']) . '; ' . $icon['name'];
    }
