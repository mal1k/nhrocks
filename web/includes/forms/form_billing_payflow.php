			...
			<form name="payflowform" target="_self" action="<?=PAYFLOW_POST_URL?>" method="post" class="custom-edit-content">
				...
				<div class="payment-action">
					...
					<? if ($payment_process == "signup") {
						$buttonGateway = "<button class=\"button button-md is-primary \" type=\"button\" id=\"payflowbutton\" onclick=\"submitOrder();\">".system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))."</button>";
					} else { ?>
                    	<button class="button button-md is-primary" type="button" id="payflowbutton" onclick="submitOrder();"><?=system_showText(LANG_BUTTON_PAY_BY_CREDIT_CARD);?></button>
					<?php } ?>
                </div>
			</form>
			<?
