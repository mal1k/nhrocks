			...
			<form name="authorizeform" target="_self" action="<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/<?=$payment_process?>/processpayment.php?payment_method=<?=$payment_method?>" method="post" class="custom-edit-content">
				...
				<div class="row default-row-biling">
					...
					<div class="form-group col-md-4">
						...
						<input id="txt-expdate" class="form-control" type="text" name="x_exp_date" value="" placeholder="<?=system_showText(LANG_LETTER_MONTH).system_showText(LANG_LETTER_MONTH)."/".system_showText(LANG_LETTER_YEAR).system_showText(LANG_LETTER_YEAR);?>" />
					</div>
					<div class="form-group col-md-4">
				...
				<div class="payment-action">
					...
					<? if ($payment_process == "signup") {
						$buttonGateway = "<button class=\"button button-md is-primary\" type=\"button\" id=\"authorizebutton\" onclick=\"submitOrder();\">".system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))."</button>";
					} else { ?>
                    <button class="button button-md is-primary" type="button" id="authorizebutton" onclick="submitOrder();"><?=system_showText(LANG_BUTTON_PAY_BY_CREDIT_CARD);?></button>
					<?php } ?>
                </div>
			</form>
