        ...
        if ($leadsArr) foreach($leadsArr as $each_lead) {
            ...
            if (is_array($auxMessage)) {
                ...
                foreach ($auxMessage as $key => $value) {
                    ...
                    $each_lead["message"] .= (defined($langKey) ? constant($langKey) : $key).($value ? ": ".$value : "")."\n";
                }
            }
			...
			<tr id="leadTR<?=$each_lead["id"];?>">
                <?php $from =  empty($each_lead['first_name']) ? ' - ' : $each_lead['first_name'] . ' ' . $each_lead['last_name'] ?>
                <td>

                    <?php if ($iconReplyForward) { ?>
                        <i class="icon-paper27" title="<?=$titleIco;?>"></i>
                    <?php } ?>

                    <?php if ($each_lead["member_id"] && string_strpos($url_base, "/".SITEMGR_ALIAS)) {
                ...
                <td><?=empty($each_lead["subject"]) ? ' - ' : $each_lead["subject"] ?></td>
