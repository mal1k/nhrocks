<?
    ...
    if (isset($filteredMatches[2])) switch ( $filteredMatches[2] )
    {
        ...
        case LISTING_FEATURE_FOLDER :
            ...
            if (LISTING_MAX_CATEGORY_ALLOWED != $listingLevelObj->getFreeCategory($level) && PAYMENTSYSTEM_FEATURE === 'on') {
                $modalCategoryInfo = "<p>".(($listingLevelObj->getFreeCategory($level) > 1) ? system_showText(LANG_LABEL_CATEGORY_PLURAL) : system_showText(LANG_LABEL_CATEGORY))." <strong>".system_showText(LANG_LABEL_FREE).": ".$listingLevelObj->getFreeCategory($level)."</strong> ".system_showText(LANG_MSG_EXTRA_CATEGORIES_COST)." <strong> ".system_showText(LANG_MSG_ADDITIONAL)." ".PAYMENT_CURRENCY_SYMBOL.$listingLevelObj->getCategoryPrice($level)."</strong>.</p>";
    ...
    if ( $matched ):
        ...
        <div class="modal custom-members-modal fade in" id="modal-categories" tabindex="-1" role="dialog" aria-labelledby="modal-categories" aria-hidden="true" >
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title"><?= system_showText( LANG_CATEGORIES_SUBCATEGS ) ?></h4>
                        <button type="button" class="button button-sm is-secondary" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-lg-offset-3">
                                ...
                                <div class="max-categories"><?= str_replace( "[max]", $modalCategoryMaxAllowed, system_showText( LANG_SELECTMAX_CATEGORIES ) ); ?></div>
                                <br>
                                <?= $modalCategoryInfo ?>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <?php } else { ?>
                            <button type="button" class="button button-md is-primary" id="action-categories"><?= system_showText( LANG_BUTTON_OK ); ?></button>
                        <?php } ?>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
