...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (validate_form("domain", $_POST, $message_domain)) {
        ...
        mixpanel_track("Added a new Site");
...
if (mysqli_num_rows($resDomain) > 0) {
    ...
    while ($rowDomain = mysqli_fetch_assoc($resDomain)) {
        if ($rowDomain["database_host"] != $_db_host || $rowDomain["database_port"] != $_db_port) {
            ...
            $link = ($GLOBALS["___mysqli_ston"] = mysqli_connect($host_link,  $rowDomain["database_username"],  $rowDomain["database_password"]));
    ...
    $databases = [];
    if (mysqli_num_rows($resDatabase)) {
        while ($rowDatabase = mysqli_fetch_assoc($resDatabase)) {
