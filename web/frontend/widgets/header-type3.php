...
?>
    <header class="header" data-type="3" is-sticky="<?=$widgetContent['stickyMenu'] === 'true' ? 'true' : 'false' ?>" is-inverse="<?=$widgetContent['backgroundColor'] === 'base' ? 'true' : 'false'?>" has-opacity="<?=$widgetContent['isTransparent'] === 'true' ? 'true' : 'false' ?>" has-mod="<?HookFire("header_check_dropdown")?>">
        ...
        <div class="navbar-mobile">
            ...
            <?php } else { ?>
                <nav class="navbar-links">
                    <a href="javascript:void(0);" data-modal="login" id="navbar-signin" class="bar-link bar-button"><?=$translator->trans($widgetContent['labelSignIn'], [], 'widgets')?></a>
                </nav>
            <?php } ?>
