    ...
    <section id="welcome">
        <div class="jumbotron">
            ...
            <button type="button" id="start-tour" class="btn btn-primary btn-lg hidden-xs">
                <small class="icon-small31"></small> <?= system_showText(LANG_SITEMGR_DASH_START_TOUR); ?></button>
            <hr>
            ...
            <div class="row">
                <div class="col-md-4 col-sm-4 col-xs-12">
                    ...
                    <a href="<?= DEFAULT_URL."/".SITEMGR_ALIAS."/design/page-editor/" ?>"
                       class="btn btn-info"
                       data-mixpanel-event='Clicked on "Customize my Directory"'>
                ...
                <div class="col-md-4 col-sm-4 col-xs-12">
                    ...
                    <a href="<?= DEFAULT_URL."/".SITEMGR_ALIAS."/promote/helpme/" ?>"
                       class="btn btn-info"
                       data-mixpanel-event='Clicked on "How to Promote"'>
                ...
                <div class="col-md-4 col-sm-4 col-xs-12">
                    ...
                    <a href="http://support.edirectory.com/" target="_blank"
                       class="btn btn-info"
                       data-mixpanel-event='Clicked on "Frequently Asked Questions"'>
                ...
                <div class="row completion-panel dashincomplete">
                    <? foreach ($arrayCompletion as $completion) { ?>
                        <div class="col-sm-6 col-lg-<?=PAYMENTSYSTEM_FEATURE === 'on' ? '3' : '4'?> clearfix">
                            <div id="step_<?= $completion["option"] ?>"
                                 class="completion-tip <?= $completion["div_class"] ?>">
                                <span id="span_<?= $completion["option"] ?>" class="tipcomplete"
                                      title="<?= $completion["check_tip"] ?>"
                                      data-auxtip="<?= system_showText(LANG_SITEMGR_DASH_STEPSDONE) ?>"
                                      data-placement="bottom" onclick="updateDashboard('<?= $completion["option"] ?>')">
                                    <i id="icon_<?= $completion["option"] ?>"
                                       class="<?= $completion["icon_class"] ?>"
                                       data-mixpanel-event='Completed step "<?= $completion["title"] ?>"'>

                                    </i>
                                </span>
                                <a href="<?= $completion["link"] ?>">
                                    <div class="pull-right"
                                         data-mixpanel-event='Clicked on "<?= $completion["title"] ?>"'>
                    ...
                    <div class="col-sm-6 col-lg-<?=PAYMENTSYSTEM_FEATURE === 'on' ? '3' : '4'?> clearfix">
                        <div id="step_<?= $completion["option"] ?>"
                            ...
                            <span id="span_<?= $completion["option"] ?>" class="tipcomplete"
                                  title="<?= $completion["check_tip"] ?>"
                                  data-auxtip="<?= system_showText(LANG_SITEMGR_DASH_STEPSDONE) ?>"
                                  data-placement="bottom" onclick="updateDashboard('<?= $completion["option"] ?>')"><i
                                        id="icon_<?= $completion["option"] ?>"
                                        class="<?= $completion["icon_class"] ?>"></i></span>
                            <a href="<?= $completion["link"] ?>">
                                <div class="pull-right">
                                    <strong><?= $completion["title"] ?></strong>
                                    <p><?= $completion["tip"] ?></p>
