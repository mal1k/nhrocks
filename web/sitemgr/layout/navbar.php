<?php
    ...
    */
    ...
    setting_get("freetrial_oid", $freetrial_oid);
    ...
    <div class="navbar navbar-inverse navbar-fixed-top" role="navigation">
        ...
        <div class="navbar-header navbar-left">
            ...
            <a  id="navBrand" class="<?=(sess_isSitemgrLogged() && string_strpos($_SERVER["PHP_SELF"], "resetpassword.php") === false && string_strpos($_SERVER["PHP_SELF"], "setlogin.php") === false ? "admin-logo" : "admin-logo")?>" href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/" target="_parent" <?=(trim(EDIRECTORY_TITLE) ? "title=\"".EDIRECTORY_TITLE."\"" : "")?>>
                <img src="<?=image_getLogoImage();?>" <?=(trim(EDIRECTORY_TITLE) ? "alt=\"".EDIRECTORY_TITLE."\"" : "")?> />
            </a>
            <? } ?>
        ...
        <?php if (PAYMENTSYSTEM_FEATURE === 'off' && setting_get('listing_limit_count')) { ?>
            <div class="trial-banner hidden-xs">
                <p><?=system_showText(LANG_SITEMGR_UPGRADE_PLAN7)?> – <a href="https://www.edirectory.com/upgrade-from-basic/?domain=<?=DEFAULT_URL?>" target="_blank"><?=system_showText(LANG_SITEMGR_UPGRADE_PLAN8)?></a></p>
            </div>
        <?php }

        if (sess_isSitemgrLogged() && string_strpos($_SERVER["PHP_SELF"], "registration.php") === false && string_strpos($_SERVER["PHP_SELF"], "resetpassword.php") === false && string_strpos($_SERVER["PHP_SELF"], "setlogin.php") === false) { ?>

        <div class="navbar-slide <?=(permission_hasSMPermSection(SITEMGR_PERMISSION_SUPERADMIN) ? "extended" : "")?>" id="navUser">
