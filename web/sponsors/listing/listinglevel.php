<?php
	...
	$cover_title = system_showText(LANG_ADD) ." ". system_showText(LANG_LISTING_FEATURE_NAME) ." - ". system_showText(LANG_LABEL_PRICE_PLURAL);
	...
	<div class="custom-level-price" data-bg="neutral">
		...
		<form action="<?= DEFAULT_URL ?>/<?= MEMBERS_ALIAS ?>/" method="get" class="level-price-actions">
			<div class="hidden">
				...
				<button class="button button-md is-primary" id="buttonContinue" type="button" onclick="document.listinglevel.submit();"><?= system_showText(LANG_BUTTON_CONTINUE) ?></button>
			</div>
		</form>
	</div>
    <?php
