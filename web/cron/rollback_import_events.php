...
$sqlIL = "SELECT `id` FROM `ImportLog` WHERE `status` = 'running' AND `module` = 'event' ORDER BY `createdAt`";
$resIL = mysqli_query( $link_domain, $sqlIL);
if (mysqli_num_rows($resIL) <= 0) {
    $sqlIL = "SELECT `id` FROM `ImportLog` WHERE `status` = 'waitrollback' AND `module` = 'event' ORDER BY `createdAt`";
    $resIL = mysqli_query( $link_domain, $sqlIL);
    if (mysqli_num_rows($resIL) > 0) {
        ...
        $num_events = 0;
        $sql = "SELECT id FROM Event WHERE import_id = ".db_formatNumber($importID);
        ...
        if ($result) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($eventObj->getNumber("id") > 0) {
                    $eventObj->Delete(SELECTED_DOMAIN_ID);
                    $num_events++;
                }
            }
        }
        ...
        $num_accounts = 0;
        $sql = "SELECT id FROM Account WHERE importID_event = ".db_formatNumber($importID)." AND domain_importID_event = ".db_formatNumber(SELECTED_DOMAIN_ID);
